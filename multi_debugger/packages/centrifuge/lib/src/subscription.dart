...
class SubscriptionImpl implements Subscription {
  ...
  void unsubscribe() async {
    ...
    final UnsubscribeRequest request = UnsubscribeRequest()..channel = channel;
    await _client.sendMessage(request, UnsubscribeResult());
    final UnsubscribeEvent event = UnsubscribeEvent();
  ...
  void _onSubscribeSuccess(SubscribeSuccessEvent event) => _subscribeSuccessController.add(event);

  void _onSubscribeError(SubscribeErrorEvent event) => _subscribeErrorController.add(event);
  ...
  Future _resubscribe({@required bool isResubscribed}) async {
    try {
      final String token = await _client.getToken(channel);
      final SubscribeRequest request = SubscribeRequest()
        ..channel = channel
        ..token = token ?? '';

      final SubscribeResult result = await _client.sendMessage(request, SubscribeResult());
      final SubscribeSuccessEvent event = SubscribeSuccessEvent.from(result, isResubscribed);
      _onSubscribeSuccess(event);
      _recover(result);
    } catch (exception) {
      if (exception is errors.Error) {
        _onSubscribeError(SubscribeErrorEvent(exception.message, exception.code));
      } else {
  ...
  void _recover(SubscribeResult result) {
    for (Publication publication in result.publications) {
      final PublishEvent event = PublishEvent.from(publication);
      addPublish(event);
    }
  }
}
