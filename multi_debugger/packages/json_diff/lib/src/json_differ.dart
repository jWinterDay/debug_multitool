...
class JsonDiffer {
  ...
  JsonDiffer.fromJson(
    ...
    if (leftJson != null && rightJson != null) {
      ...
      throw const FormatException('JSON must not be null');
    }
  }
  ...
  DiffNode _diffObjects(Map<String, Object> left, Map<String, Object> right, List<Object> path) {
    ...
    left.forEach((String key, Object leftValue) {
      ...
      if (atomics.contains(key) && leftValue.toString() != rightValue.toString()) {
        ...
        node[key] = _diffObjects(leftValue.cast<String, Object>(), rightValue.cast<String, Object>(), [...path, key]);
  ...
  bool _deepEquals(dynamic e1, dynamic e2) => const DeepCollectionEquality.unordered().equals(e1, e2);

  DiffNode _diffLists(List<Object> left, List<Object> right, String parentKey, List<Object> path) {
    final node = DiffNode(path);
    var leftHand = 0;
    var leftFoot = 0;
    var rightHand = 0;
      ...
      if (!_deepEquals(left[leftFoot], right[rightFoot])) {
        ...
        if (!foundMissing) {
          ...
          if (parentKey != null &&
            ...
            node[leftFoot] = _diffObjects(
                leftObject.cast<String, Object>(), rightObject.cast<String, Object>(), [...path, leftFoot]);
          } else if (leftObject is List && rightObject is List) {
            node[leftFoot] = _diffLists(leftObject, rightObject, null, [...path, leftFoot]);
          } else {
            node.changed[leftFoot] = [leftObject, rightObject];
          }
        }
    ...
    final removedFiltered = node.removed.entries.where((e) {
      final added = node.added.removeFirstWhere((key, value) => _deepEquals(e.value, value));
