...
Middleware<V, B, A> createEpicMiddleware<V extends Built<V, B>, B extends Builder<V, B>, A extends ReduxActions>(
  ...
  // ignore: close_sinks
  final StreamController<Action<dynamic>> _actions = StreamController.broadcast(sync: true);
  // ignore: close_sinks
  ...
  return (MiddlewareApi<V, B, A> mwApi) => (next) => (action) {
        if (!_isSubscribed) {
          _epics.stream
              .transform<dynamic>(
                SwitchMapStreamTransformer<Epic<V, B, A>, dynamic>((epic) => epic(_actions.stream, mwApi)),
              )
...
class EpicBuilder<V extends Built<V, B>, B extends Builder<V, B>, A extends ReduxActions> {
  ...
  void add<T>(ActionName<T> actionName, EpicHandler<V, B, A, T> handler) {
    _epics.add((Stream<Action<dynamic>> action, MiddlewareApi<V, B, A> mwApi) =>
        handler(action.where((a) => a.name == actionName.name).cast<Action<T>>(), mwApi));
  }
